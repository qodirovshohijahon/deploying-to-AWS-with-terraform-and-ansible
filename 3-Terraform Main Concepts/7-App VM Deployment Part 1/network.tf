...
resource "aws_subnet" "subnet_1" {
  provider               = aws.region-master
  aws_availability_zones = element(data.aws_availability_zones.azs.names, 0)
  vpc_id                 = aws_vpc.vpc_master.id
...
resource "aws_route_table" "internet_route" {
  ...
  route {
    cidr_block                = "192.168.1.0/24"
    vpc_peering_connection_id = aws_vpc_peering_connection.ueast1-uswest2.id
  }
  lifecycle {
    ignore_changes = all
  }
...
resource "aws_main_route_table_association" "set-worker-default-rt-assoc" {
  provider       = aws.region-worker
  vpc_id         = aws_vpc.vpc_orgeon.id
  route_table_id = aws_route_table.internet_oregon.id
}
